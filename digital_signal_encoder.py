...
class DigitalSignalEncoder:
    def __init__(self, root):
        ...
        # Button frame for Generate and Clear buttons
        ...
        instructions = (
            ...
            "2. Click 'Generate All Signals' to see the encoding results\n"
            "3. Each plot shows a different encoding technique used in digital communication"
        )
    ...
    def nrz_i(self, data):
        ...
        for bit in data:
            ...
            signal.append(1 if current_level == 1 else -1)
        signal.append(signal[-1])
        return signal
    ...
    def differential_manchester(self, data):
        ...
        for bit in data:
            if bit == '1':
                # No transition at start of bit, transition in middle
                signal.extend([last_level, last_level, -last_level, -last_level])
                t.extend([i, i + 0.5, i + 0.5, i + 1])
                last_level = -last_level
            else:
                # Transition at start of bit, transition in middle
                signal.extend([-last_level, -last_level, last_level, last_level])
                t.extend([i, i + 0.5, i + 0.5, i + 1])
            i += 1
        return np.array(t), np.array(signal)
